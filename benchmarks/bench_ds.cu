...
template <typename KeyOp>
void run_keygen(GpuProfiler& profiler, const std::string& variant, unsigned int batch,
    ...
    profiler.stop_profile(variant + " Key Generation", start, stop, batch);
...
template <typename SignOp>
void run_sign(GpuProfiler& profiler, const std::string& variant, unsigned int batch,
    ...
    profiler.stop_profile(variant + " Signing", start, stop, batch);
...
template <typename VerifyOp>
void run_verify(GpuProfiler& profiler, const std::string& variant, unsigned int batch,
    ...
    profiler.stop_profile(variant + " Verification", start, stop, batch);
...
int main() {
    ...
    printf("\n=== ML-DSA-44 Benchmark ===\n");
    run_keygen<MLDSA44KeyOp>(profiler, "ML-DSA-44", batch, public_keys44, secret_keys44);
    run_sign<MLDSA44SignOp>(profiler, "ML-DSA-44", batch, signatures44, messages, secret_keys44, message_size);
    run_verify<MLDSA44VerifyOp>(profiler, "ML-DSA-44", batch, valids44, signatures44, messages, public_keys44, message_size);
    ...
    printf("\n=== ML-DSA-65 Benchmark ===\n");
    run_keygen<MLDSA65KeyOp>(profiler, "ML-DSA-65", batch, public_keys65, secret_keys65);
    run_sign<MLDSA65SignOp>(profiler, "ML-DSA-65", batch, signatures65, messages, secret_keys65, message_size);
    run_verify<MLDSA65VerifyOp>(profiler, "ML-DSA-65", batch, valids65, signatures65, messages, public_keys65, message_size);
    ...
    printf("\n=== ML-DSA-87 Benchmark ===\n");
    run_keygen<MLDSA87KeyOp>(profiler, "ML-DSA-87", batch, public_keys87, secret_keys87);
    run_sign<MLDSA87SignOp>(profiler, "ML-DSA-87", batch, signatures87, messages, secret_keys87, message_size);
    run_verify<MLDSA87VerifyOp>(profiler, "ML-DSA-87", batch, valids87, signatures87, messages, public_keys87, message_size);
